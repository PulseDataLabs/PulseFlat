...
from utils import agora_brt, get_logger, nova_session, salvar_csv
        ...
        try:
            ...
            for col in num_cols:
                if col in df_cleaned.columns:
                    ...
                    df_cleaned.loc[mask, col] = s.str.replace(
                        ".", "", regex=False
            ...
            if not is_pipeline:
                ...
                print_done(
                    f"{len(registros)} registros salvos em {self.output_file.name}",
                    elapsed=elapsed,
                )
