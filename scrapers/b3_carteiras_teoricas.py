...
def _mapear(item: dict, data_captura: str, index: str, label: str) -> dict:
    return {
        "data_captura": data_captura,
        "indice": index,
        "indice_nome": label,
        "codigo_ativo": limpar(
            item.get("cod") or item.get("ticker") or item.get("code")
        ),
        "nome_ativo": limpar(
        ...
        "quantidade_teorica": _limpar_int(
            limpar(
                ...
                or item.get("qtyTheoretical")
            )
        ),
        "participacao_pct": _limpar_float(
            limpar(item.get("part") or item.get("participation") or item.get("weight"))
        ),
        "reducao_capital": _limpar_float(
            limpar(item.get("reductionPct") or item.get("capitalReduction"))
        ),
        "segmento": limpar(item.get("segment") or item.get("setor")),
    }


def _capturar_indice(
    session, index: str, segment: str, label: str, data_captura: str
) -> list[dict]:
...
def capturar() -> list[dict]:
    ...
    log.info(
        f"{len(todos)} registros | {len(INDICES)-len(erros)}/{len(INDICES)} índices OK"
    )
    if erros:
