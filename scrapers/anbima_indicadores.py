...
def capturar() -> list[dict]:
    ...
    m_igpm_proj = re.search(
        r"IGP-M\s*\d*\s*Proje..o\s*\(([^)]+)\)", texto, re.IGNORECASE
    )
    ...
    m_ipca_proj = re.search(
        r"IPCA\s*\d*\s*Proje..o\s*\(([^)]+)\)", texto, re.IGNORECASE
    )
    ...
    for nome, padrao, dt_ref, unidade in [
        (
            "IGP-M Número Índice",
            r"IGP-M\s*\([^)]+\)\s*\d*\s*N.mero\s*.ndice\s*([\d\.,]+)",
            dt_igpm,
            "índice",
        ),
        (
            "IGP-M Var % mês",
            r"IGP-M\s*\([^)]+\)\s*\d*\s*.*?Var\s*%\s*no\s*m[eê]s\s*(-?\d+[\.,]\d+)",
            dt_igpm,
            "%",
        ),
        (
            "IGP-M Projeção",
            r"IGP-M\s*\d*\s*Proje..o\s*\([^)]+\)\s*(-?\d+[\.,]\d+)",
            dt_igpm_proj,
            "%",
        ),
        (
            "IPCA Número Índice",
            r"IPCA\s*\([^)]+\)\s*\d*\s*N.mero\s*.ndice\s*([\d\.,]+)",
            dt_ipca,
            "índice",
        ),
        (
            "IPCA Var % mês",
            r"IPCA\s*\([^)]+\)\s*\d*\s*.*?Var\s*%\s*no\s*m[eê]s\s*(-?\d+[\.,]\d+)",
            dt_ipca,
            "%",
        ),
        (
            "IPCA Projeção",
            r"IPCA\s*\d*\s*Proje..o\s*\([^)]+\)\s*(-?\d+[\.,]\d+)",
            dt_ipca_proj,
            "%",
        ),
    ]:
    ...
    for nome, padrao in [
        (
            ...
            r"Dolar Comercial Compra.*?(\d{2}/\d{2}/\d{4}).*?(\d+[\.,]\d+)",
        ),
        (
            "Dólar Comercial Venda",
            r"D.lar Comercial Venda.*?(\d{2}/\d{2}/\d{4}).*?(\d+[\.,]\d+)",
        ),
    ...
    for i, m in enumerate(
        re.findall(r"FDS.*?(\d{2}/\d{2}/\d{4}).*?(\d+[\.,]\d+)", texto, re.DOTALL)[:2],
        1,
    ):
