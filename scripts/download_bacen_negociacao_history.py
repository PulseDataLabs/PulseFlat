...
from utils.parsers import enriquecer, rows_from_zip
        ...
        for tentativa in range(1, 4):
            try:
                ...
                if resp.status_code == 404:
                    log.info(
                        f"  [404] Arquivo para {year_month} não encontrado (pulando)."
                    )
                    resp = None
                    break
            ...
            try:
                ...
                if rows:
                    ...
                    enriched, _ = enriquecer(
                        dataset_id, rows, data_captura_override=last_day_str
                    )
                    all_enriched.extend(enriched)
                    log.info(
                        f"  [OK] {len(enriched)} registros extraídos e enriquecidos."
                    )
                else:
