...
def run():
    parser = argparse.ArgumentParser(description="Importador de Histórico de Negócios de Ações da B3")
    parser.add_argument("--start-date", default="2026-01-01", help="Data de início (AAAA-MM-DD)")
    parser.add_argument("--end-date", default=date.today().strftime("%Y-%m-%d"), help="Data de término (AAAA-MM-DD)")
    ...
    for i, dt in enumerate(datas_uteis, 1):
        ...
        if todos_do_dia:
            ...
            salvar_csv(
                ...
                registros=df_day,
                cabecalho=[
                    "data_captura", "data_referencia", "codigo_ativo", "isin", "sgmt_nm", "mkt",
                    "preco_abertura", "preco_minimo", "preco_maximo", "preco_medio", "preco_ultimo",
                    "osc", "trad_qty", "fin_instrm_qty", "ntl_fin_vol"
                ],
