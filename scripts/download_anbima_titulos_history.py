...
from utils.base import salvar_csv, nova_session, get_logger, limpar
...
def run():
    ...
    log.info(f"Total de {len(dates_to_fetch)} dias de semana para buscar no histórico desde 2020.")
        ...
        for future in as_completed(futures):
            ...
            if res:
                ...
                if completed_count % 100 == 0 or completed_count == total_count:
                    log.info(f"  Progresso: {completed_count}/{total_count} datas processadas. {len(all_records)} registros carregados.")
