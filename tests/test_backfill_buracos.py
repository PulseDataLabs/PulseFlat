...
class TestBackfillBuracos:
    def test_backfill_dry_run(self):
        ...
        with patch.object(main_module, "load_entity_dates", mock_load), patch.object(
            main_module, "check_gaps", mock_check_gaps
        ), patch.object(
            main_module.importlib, "import_module", mock_import
        ), patch.object(
            main_module.Path, "exists", mock_exists
        ):
    ...
    def test_backfill_execution_yahoo(self):
        ...
        mock_check_gaps = MagicMock(
            return_value={("*",): ["2026-06-02", "2026-06-03", "2026-06-04"]}
        )
