...
def get_isin_zip(session) -> bytes:
    ...
    if not file_id:
        raise RuntimeError(
            "Não foi possível encontrar o ID do arquivo geralPt nos metadados da B3."
        )
...
def get_company_seeds(session) -> list[dict]:
    ...
    while True:
        ...
        for it in items:
            code_cvm = limpar(
                it.get("codeCVM") or it.get("codeCvm") or it.get("codCvm")
            )
            trading_name = limpar(
                it.get("companyName") or it.get("tradingName") or it.get("name")
            )
            issuing = limpar(
                it.get("issuingCompany") or it.get("code") or it.get("acronym")
            )
